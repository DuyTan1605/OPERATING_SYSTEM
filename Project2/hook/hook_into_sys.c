...
asmlinkage int	write(unsigned int fd,const char __user *buf,size_t count )
{
	if (strcmp(current->comm ,"hooksys_test")==0)
	{
		...
		printk(KERN_INFO "[Sys_write]Process excute: %s\n",current->comm);
		printk(KERN_INFO"[Sys_write]Wrote into file: %s\n",pathname);
		printk(KERN_INFO"[Sys_write]Wrote: %ld bytes\n",count);
...
asmlinkage long	open(const char __user * pathname, int flags, mode_t mode )
{
	...
	if (strcmp(current->comm ,"hooksys_test")==0)
	{
 		...
 		printk( KERN_INFO "[Sys_open]Process excute: %s\n",current->comm);
		printk( KERN_INFO "[Sys_open]Opened file: %s\n",pathname);
	}
...
static int init_mod(void)
{
	...
	printk(KERN_INFO"LOADED MODULE HOOKING SUCCESSFULLY!!");
...
{
	printk(KERN_INFO"UNLOADED MODULE HOOKING SUCCESSFULLY!!");
 	allow_write();
